...
template <typename value_type>
vector<value_type>::vector(size_type n) : data_ptr(new value_type[n]), Size(n), Capacity(n) {
    std::fill(data_ptr, data_ptr + n, value_type());
}

template <typename value_type>
vector<value_type>::vector(const vector &v) : data_ptr(new value_type[v.Capacity]), Size(v.Size), Capacity(v.Capacity) {
    std::copy(v.data_ptr, v.data_ptr + v.Size, data_ptr);
}

template <typename value_type>
vector<value_type>::vector(vector &&v) : data_ptr(v.data_ptr), Size(v.Size), Capacity(v.Capacity) {
    v.data_ptr = nullptr;
    v.Size = 0;
    v.Capacity = 0;
}
...
template <typename value_type>
vector<value_type>::vector(std::initializer_list<value_type> const &items) : data_ptr(new value_type[items.size()]), Size(items.size()), Capacity(items.size()) {
...
template <typename value_type>
vector<value_type>::reference vector<value_type>::operator=(vector &&v) {
    if (this != &v) {
        delete[] data_ptr;
...
template <typename value_type>
typename vector<value_type>::iterator vector<T>::insert(iterator pos, const_reference value) {
